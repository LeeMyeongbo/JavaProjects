...
final public class SettingDataManager {
    ...
    public static void loadSettingsFromFile() {
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream("C:\\Mediaplayer\\setting.dat"))) {
            ...
            LOG.info("loadSettingsFromFile : read setting.dat successfully from C:\\Mediaplayer");
        } catch (FileNotFoundException e) {
            setDefaultSettings();
            saveSettingsOnFile();
        } catch (IOException | ClassNotFoundException e) {
            LOG.fatal("loadSettingsFromFile : cannot read setting.dat from C:\\Mediaplayer : " + e.getMessage());
            throw new RuntimeException(e);
        }
    }

    private static void setDefaultSettings() {
        LOG.info("setDefaultSettings : There is no setting.dat in C:\\Mediaplayer, so set to default value");
        putMoveTimeValue(10);
    ...
    public static void saveSettingsOnFile() {
        ...
        if (path.mkdir()) {
            LOG.info("saveSettingsOnFile : MediaPlayer dir created in C:\\");
        }
        ...
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(AbsolutePath))) {
            ...
            oos.flush();
            LOG.info("saveSettingsOnFile : setting.dat saved successfully in C:\\Mediaplayer");
        } catch (IOException e) {
            LOG.fatal("saveSettingsOnFile : cannot save setting.dat in C:\\Mediaplayer : " + e.getMessage());
            throw new RuntimeException(e);
        }
    }
