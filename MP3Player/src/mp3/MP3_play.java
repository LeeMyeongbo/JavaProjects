...
class MP3_play extends JFrame implements ActionListener { // JFrame 상속 및 ActionListener 구현
	...
	private ImageIcon image = new ImageIcon("background.png"); // 배경
	private JPanel jp = new JPanel() { // 패널에다 위의 배경을 그림
		private static final long serialVersionUID = 1L;
	...
	private ArrayList<File> filelists = new ArrayList<File>(); // mp3 파일 저장용 ArrayList
	private String[] but = { "재생.png", "이전곡.png", "다음곡.png", "일시정지.png" }; // 4가지 버튼 이미지 파일 이름
	private String[] opt = {"전체 한 번씩.png", "전체반복.png", "셔플.png"}; // 3가지 재생 옵션 파일 이름
	private ImageIcon[] buttonimgs = new ImageIcon[4]; // 재생, 일시정지, 이전곡, 다음곡 버튼 이미지
	private ImageIcon[] optionimgs = new ImageIcon[3]; // 전체 한 번 재생, 전체 반복 재생, 셔플 재생 버튼 이미지
	private JButton[] jb = new JButton[3]; // 재생, 일시정지, 이전곡, 다음곡 버튼
	private JButton[] options = new JButton[3]; // 전체 한 번 재생, 전체 반복 재생, 셔플 재생 버튼
	private JSlider js1 = new JSlider(JSlider.HORIZONTAL); // 노래 재생 상태를 나타내는 js1 슬라이더
	private JSlider js2 = new JSlider(JSlider.VERTICAL); // 볼륨 크기를 나타내는 js2 슬라이더
	private JLabel presentimes = new JLabel(); // 현재 재생 중인 시간 표시
	private JLabel lastimes = new JLabel(); // 총 노래 길이 표시
	private JLabel song = new JLabel(); // 재생 중인 노래 제목 표시
	private JLabel vol = new JLabel(); // 볼륨 표시
	private JTextField jt = new JTextField(); // 검색창
	private JButton searchb = new JButton("검색"); // 검색 버튼
	private JButton addb = new JButton("추가"); // 추가 버튼(디렉토리에서 노래를 PlayList에 추가함)
	private JButton deleteb = new JButton("삭제"); // 삭제 버튼(PlayList에서 노래 삭제)
	private JButton savelists = new JButton("목록 내보내기"); // 재생목록 내보내기 버튼
	private JButton openlists = new JButton("목록 불러오기"); // 재생목록 불러오기 버튼
	private JFileChooser chooser = new JFileChooser("C:\\"); // 추가 버튼을 눌렀을 때 파일 디렉토리를 나타냄
	private FileNameExtensionFilter filter = new FileNameExtensionFilter(".mp3, .dat", "mp3", "dat"); // JFileChooser에서 .mp3, .dat 파일만 필터링
	private DefaultListModel<String> listmodel = new DefaultListModel<String>(); // JList가 리스트화할 기본 ListModel 설정
	private JList<String> jl = new JList<String>(); // PlayList에 추가된 노래 목록 나타냄
	private JScrollPane scroll = null; // PlayList에 스크롤바를 추가(목록이 많아지면 스크롤바로 보이지 않는 목록들까지 볼 수 있음)
	...
	private LinkedList<Integer> order = new LinkedList<Integer>(); // 셔플 재생일 때 재생 순서를 정하기 위함
	// 노래가 시작되었을 때 주기적으로 처리할 작업 명시
	private Timer t = new Timer(100, e -> { // 0.1초 동안 반복해서 해당 actionPerformed를 호출 (t.start() 메소드 호출 시작)
	...
	public MP3_play() {
		...
		js2.addChangeListener(new ChangeListener() { // 볼륨 조절 슬라이더의 노브에 변화가 일어났을 때 발생되는 이벤트 처리
			@Override
	...
	public void setFileChooserFont(Component[] comp) {
		for(int i = 0; i < comp.length; i++) {
			...
			try {
				comp[i].setFont(new Font("경기천년제목 Light", Font.BOLD, 15));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
			...
			if (pressedbutton.equals(addb)) { // 추가 버튼 클릭했을 때
				...
				if(option == JFileChooser.APPROVE_OPTION) {
					...
					for(int i = 0; i < selectedfiles.length; i++) {
						try {
							...
							filelists.add(selectedfiles[i]); // filelists에 파일 추가
						} catch (Exception e1) {
				...
				for(int i = 0; i < selected.length; i++) {
					...
					if(m != null) // 노래를 재생 중이었을 때
						if(m.getSource().toString().equals(deletingfile.toURI().toString())) { // 삭제할려는 노래가 재생 중 일때
							t.stop();
							player.stop();
							player = null;
							m = null;
